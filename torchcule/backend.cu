...
BfsAtariEnv::
BfsAtariEnv(const cule::atari::rom& cart,
            const size_t num_envs,
            const size_t noop_reset_steps)
    : super_t(cart, num_envs, noop_reset_steps),
      use_cuda(false),
     ...
     bool* doneBuffer)
{
    if(use_cuda)
    {
        super_t::step(get_policy<cule_policy>(), fire_reset, num_envs, doneBuffer);
    }
    else
    {
        super_t::step(get_policy<agency::parallel_execution_policy>(), fire_reset, num_envs, doneBuffer);
    }
}
         ...
         int32_t* livesBuffer)
{
    if(use_cuda)
    {
        super_t::get_data(get_policy<cule_policy>(), episodic_life, num_envs, doneBuffer, rewardsBuffer, livesBuffer);
    }
    else
    {
        super_t::get_data(get_policy<agency::parallel_execution_policy>(), episodic_life, num_envs, doneBuffer, rewardsBuffer, livesBuffer);
    }
}
...
#include <cule/atari/bfs_wrapper.cpp>
