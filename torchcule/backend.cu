     ...
     const int32_t* indexBuffer)
{
    if(use_cuda)
    {
        super_t::step(get_policy<cule_policy>(), fire_reset, expand, playerABuffer, playerBBuffer, doneBuffer, indexBuffer);
    }
    else
    {
        super_t::step(get_policy<agency::parallel_execution_policy>(), fire_reset, expand, playerABuffer, playerBBuffer, doneBuffer, indexBuffer);
    }
}
         ...
         int32_t* livesBuffer)
{
    if(use_cuda)
    {
        super_t::get_data(get_policy<cule_policy>(), episodic_life, scale, doneBuffer, rewardsBuffer, livesBuffer);
    }
    else
    {
        super_t::get_data(get_policy<agency::parallel_execution_policy>(), episodic_life, scale, doneBuffer, rewardsBuffer, livesBuffer);
    }
}
...
void
AtariEnv::
update_frame_states(void)
{
    if(use_cuda)
    {
        super_t::update_frame_states(get_policy<cule_policy>());
    }
    else
    {
        super_t::update_frame_states(get_policy<agency::parallel_execution_policy>());
    }
}
