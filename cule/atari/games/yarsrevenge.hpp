 ...
 void step(State& s)
{
    ...
    // update the reward
    int score = getDecimalScore(s, 0xE2, 0xE1, 0xE0);
    int reward = score - s.m_score;
    s.m_reward = reward;
    s.m_score = score;

    // update terminal status
    int lives_byte = ram::read(s, 0x9E) >> 4;
...
{
    return cule::atari::ram::read(s.ram, 0x9E) >> 4;
}
...
{
    return cule::atari::games::getDecimalScore(s, 0xE2, 0xE1, 0xE0);
}
