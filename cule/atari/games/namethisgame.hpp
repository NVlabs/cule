 ...
 void step(State& s)
{
    ...
    // update the reward
    int score = getDecimalScore(s, 0xC6, 0xC5, 0xC4);
    int reward = score - s.m_score;
    s.m_reward = reward;
    s.m_score = score;

    // update terminal status
    s.m_lives = (ram::read(s, 0xC7) & 0x7);
...
{
    return cule::atari::ram::read(s.ram, 0xC7) & 0x7;
}
...
{
    return cule::atari::games::getDecimalScore(s, 0xC6, 0xC5, 0xC4);
}
