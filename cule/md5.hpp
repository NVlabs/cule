...
static void MD5_memcpy(POINTER output, POINTER input, const unsigned int len)
{
...
static void MD5_memset(POINTER output, const int value, const unsigned int len)
{
    for(unsigned int i = 0; i < len; i++)
    {
...
} // end namespace md5_details

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void MD5(const uint8_t* buffer, const uint32_t length, uint8_t * md5)
{
    md5_details::MD5_CTX context;
    md5_details::MD5Init(&context);
    md5_details::MD5Update(&context, buffer, length);
    md5_details::MD5Final(md5, &context);
}
